...
import { AlertCircle, CheckCircle2, MessageSquare, Send, Zap } from 'lucide-react';
...
const Claims = () => {
  ...
  const [errors, setErrors] = useState(parsedData?.validation?.errors || parsedData?.parsed?.errors || parsedData?.errors || []);
  ...
  const handleFullValidate = async () => {
    ...
    try {
      ...
      setErrors(res.data.errors || res.data.validation?.errors || []);
    } catch (err) {
      console.error(err);
    } finally {
      setValidating(false);
    ...
    try {
      const res = await axios.post('/api/chat', {
        ...
        parsed_context: {
          ...
          validation_errors: parsedData.validation?.errors || parsedData.errors || [],
          validation_warnings: parsedData.validation?.warnings || parsedData.warnings || []
        }
      });
  ...
  return (
    <div className="flex-grow flex flex-col h-screen pt-24 pb-6 px-6 relative">
      ...
      {/* Background ambient glows specific to Claims */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 h-full gap-6 relative z-10">
        ...
        <div className="flex flex-col gap-6 overflow-hidden">
          ...
          {/* Top: Errors */}
          <div className="glass-panel flex flex-col h-1/3 min-h-[220px] overflow-hidden animate-fade-in-up shadow-lg" style={{animationDelay: '0.3s'}}>
            <div className="px-5 py-4 bg-white/50 border-b border-slate-200/50 flex items-center justify-between backdrop-blur-sm">
              <h3 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                <AlertCircle className="w-4 h-4 text-danger" /> Validation Status
              </h3>
              <button
            ...
            <div className="flex-grow overflow-auto p-4 space-y-2 custom-scrollbar">
              {errors.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-2">
                  <CheckCircle2 className="w-8 h-8 text-success/50" />
                  <p className="text-sm font-medium">No validation errors detected.</p>
                </div>
              ) : (
                errors.map((err, idx) => (
                  <div key={idx} className="p-3 bg-danger/5 border border-danger/20 rounded-xl flex gap-3 text-sm">
                    <AlertCircle className="w-4 h-4 text-danger flex-shrink-0 mt-0.5" />
                    <span className="text-slate-700 font-medium">
                      {typeof err === 'object' ? err.description || err.message || JSON.stringify(err) : err}
                    </span>
                  </div>
                ))
              )}
          ...
          {/* Bottom: Chat */}
          <div className="glass-panel flex-grow flex flex-col overflow-hidden animate-fade-in-up shadow-lg" style={{animationDelay: '0.4s'}}>
            <div className="px-5 py-4 bg-white/50 border-b border-slate-200/50 flex flex-col backdrop-blur-sm">
              <h3 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                <MessageSquare className="w-4 h-4 text-accent" /> ClaimCraft AI Assistant
              </h3>
            </div>
